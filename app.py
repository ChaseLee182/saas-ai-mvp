...
# --- 配置 Streamlit 页面 ---
st.set_page_config(
    page_title="SaaS AI 文案生成器 (密钥快速修复)",
    layout="wide",
...
# 1. 尝试从 Secrets 中获取密钥 (优先)
# 2. 如果 Secrets 失败，则使用侧边栏的输入框
...
def initialize_gemini_client(api_key):
    ...
    try:
        ...
        # st.error(f"AI 客户端初始化失败: {e}") # 避免在启动时报错
        return None
...
with st.sidebar:
    ...
    try:
        # 这一行是 Streamlit Cloud 官方推荐的读取方法
        # 如果您在 Secrets 中配置了，这里会读取到
        secret_key = st.secrets["GEMINI_API_KEY"]
    except KeyError:
        secret_key = None

    # 如果 Secrets 中没有密钥，或者您想临时替换，则显示输入框
    user_api_key = st.text_input(
        "输入您的 Gemini API 密钥",
        value=secret_key if secret_key else "",
        type="password",
        help="密钥将用于直接调用 AI 服务。一旦输入，应用将尝试使用它。"
    )

    # 确定最终使用的密钥
    final_api_key = user_api_key if user_api_key else secret_key
    
    # 初始化客户端
    ai_client = initialize_gemini_client(final_api_key)

    if not ai_client:
        st.warning("请在上方输入有效的 Gemini API 密钥，或检查 Secrets 配置。")
    else:
        st.success("AI 客户端已准备就绪！")
...
def generate_content(client, log_content, style_prompt, format_prompt):
    # ... [此处为 generate_content 函数的完整代码，与我上一步提供的代码一致] ...
    """
    调用 Gemini API 生成内容。
...
if st.button("✨ 生成专业内容！", type="primary"):
    if not technical_log:
        ...
        st.warning("请在左侧侧边栏中输入有效的 API 密钥。")
    else:
        # 组合风格和格式描述，传递给 AI
